...
interface RolloverScenarioState {
  ...
  checkDrift: (id: number) => Promise<SnapshotDrift>
  refreezeScenario: (id: number) => Promise<RolloverScenario>
...
export const useRolloverScenarioStore = create<RolloverScenarioState>((set, get) => {
  ...
  return {
    items: [], total: 0, status: 'idle', error: '', active: null, drift: null,
    ...
    createScenario: async (input) => {
      ...
      set({ items: [created, ...get().items], total: get().total + 1, active: created, drift: null })
      return created
    },
    ...
    checkDrift: async (id) => {
      const drift = await rolloverScenarioApi.drift(id)
      if (get().active?.id === id) set({ drift })
      return drift
    },
    refreezeScenario: async (id) => {
      const updated = await rolloverScenarioApi.refreeze(id)
      merge(updated)
      const drift = await rolloverScenarioApi.drift(id)
      if (get().active?.id === id) set({ drift })
      return updated
    ...
    select: (scenario) => set({ active: scenario, drift: null }),
  }
})
